...
1. **Upload** one or more `.md` (or `.txt`) files into this folder via the GitHub web interface ("Add file → Upload files") or by committing them from your machine.
...
- Only `.md` and `.txt` files are processed; other file types should be placed directly in `assets/`.
